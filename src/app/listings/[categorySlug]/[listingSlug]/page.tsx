...
export default async function SingleListingPage({ params }: PageProps) {
  ...
  const [listing, relatedListings, categoryDetails] = await Promise.all([
    getListingBySlug(categorySlug, listingSlug, ListingContext.LOCAL),
    getRelatedListings(categorySlug, listingSlug, 3, ListingContext.LOCAL),
  ...
  const breadcrumbItems = [
    ...
    { label: categoryDetails.name, href: `/listings/${categorySlug}` },
